...
class DumyDataStep(WorkflowStepMountPoint):
    ...
    def getPortData(self, index):
        ...
        import json
        with open('modresult.json', 'r') as fp:
            ecg_dict = json.load(fp)
        ecg_dict_new = self._order_node_numbers(ecg_dict)
        self._portData0 = ecg_dict_new


        return self._portData0 # ecg_grid_points

    def _order_node_numbers__OLD__(self, description):
        ...
        while len(sortx) != 0:
            if j == 0:
                miny = 0
            elif (j*8-len(storage_list)-1) <= 0:
                miny = j*8
            else:
                miny = j*8-len(storage_list)-1
